...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Deleting field 'Appointment.location'
        db.delete_column('healers_appointment', 'location_id')

        # Deleting field 'Appointment.healer'
        db.delete_column('healers_appointment', 'healer_id')

        # Deleting field 'Appointment.weekly'
        db.delete_column('healers_appointment', 'weekly')

        # Deleting field 'Appointment.id'
        db.delete_column('healers_appointment', 'id')

        # Adding field 'Appointment.healertimeslot_ptr'
        db.add_column('healers_appointment', 'healertimeslot_ptr', self.gf('django.db.models.fields.related.OneToOneField')(default=1, to=orm['healers.HealerTimeslot'], unique=True, primary_key=True), keep_default=False)


    def backwards(self, orm):
        
        # We cannot add back in field 'Appointment.start'
        raise RuntimeError(
            "Cannot reverse this migration. 'Appointment.start' and its values cannot be restored.")

        # We cannot add back in field 'Appointment.end'
        raise RuntimeError(
            "Cannot reverse this migration. 'Appointment.end' and its values cannot be restored.")

        # Adding field 'Appointment.location'
        db.add_column('healers_appointment', 'location', self.gf('django.db.models.fields.related.ForeignKey')(default=None, to=orm['healers.Location'], null=True), keep_default=False)

        # We cannot add back in field 'Appointment.healer'
        raise RuntimeError(
            "Cannot reverse this migration. 'Appointment.healer' and its values cannot be restored.")
        ...
        # We cannot add back in field 'Appointment.id'
        raise RuntimeError(
            "Cannot reverse this migration. 'Appointment.id' and its values cannot be restored.")

        # Deleting field 'Appointment.healertimeslot_ptr'
        db.delete_column('healers_appointment', 'healertimeslot_ptr_id')
    ...
    models = {
        ...
        'healers.appointment': {
            'Meta': {'object_name': 'Appointment', '_ormbases': ['healers.HealerTimeslot']},
            'client': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['clients.Client']"}),
            'healertimeslot_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': "orm['healers.HealerTimeslot']", 'unique': 'True', 'primary_key': 'True'})
        },
        ...
        'healers.location': {
            ...
            'room': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['healers.Room']", 'blank': 'True'}),
