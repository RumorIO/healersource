...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        ClientVideo = orm['clients.ClientVideo']
        VIDEO_TYPE_CLIENT = 2

        client_ids = set(ClientVideo.objects.values_list('client_id', flat=True))

        for client_id in client_ids:
            videos = ClientVideo.objects.filter(client_id=client_id).order_by("date_added")
            i = 1
            for video in videos:
                video.order = i
                video.videotype = VIDEO_TYPE_CLIENT
                video.save()
                i += 1
