...
class Migration(DataMigration):
    ...
    def update_complete(self, instance, save=False):
        questions = instance.form.questions.all()
        instance.complete = False
        check = [(question.results.filter(answer__client=instance.client).count() > 0) for question in questions]
        if questions.count() == len(check) and set(check) == set([True]):
    ...
    def forwards(self, orm):
        ...
        Answer = orm['intake_forms.Answer']
        for answer in Answer.objects.all():
            self.update_complete(answer, save=True)
