...
# class PaymentEventProcessingException(models.Model):

# 	event = models.ForeignKey("PaymentEvent", null=True)
...
# class PaymentEvent(StripeObject):
...
# 				PaymentEventProcessingException.log(
# 					data=e.http_body,
# 					exception=e,
# 					event=self
# 				)
...
# 		if signal:
# 			return signal.send(sender=PaymentEvent, event=self)
...
class Customer(StripeObject):
	...
	card_stripe_id = models.CharField(max_length=255)  # should be unique. Skipped for now because the default value can't be set.
	date_purged = models.DateTimeField(null=True, editable=False)
	payment_schedule = models.PositiveSmallIntegerField(default=0)
	payment_notes = models.PositiveSmallIntegerField(default=0)
	payment_booking = models.PositiveSmallIntegerField(default=0)
	is_custom_schedule_plan = models.BooleanField(default=False)
	...
	# used in tests
	def get_total_charge_amount(self):
		return float(Charge.objects.filter(customer=self).aggregate(
			Sum('amount')).values()[0])

	# used in tests
	def get_upcoming_invoices_total_amount(self):
		upcoming_invoice = stripe.Invoice.upcoming(customer=self.stripe_id)
		return sum([x['amount'] for x in upcoming_invoice['lines']['data']])/100.0
	...
	def can_charge(self):
		return self.card_fingerprint != ''

	def get_payment(self, plan_type, subscription=None):
		if subscription is None:
			subscription = self.get_subscription(plan_type)
		return subscription.plan.amount / 100.0

	def get_subscription(self, plan_type):
		def get_plan_type(plan_id):
			return plan_id.split('_')[0]

		if not hasattr(self.stripe_customer, 'subscriptions'):
			return
			# raise stripe.error.StripeError('No subscription attribute.')

		current_subscriptions = self.stripe_customer.subscriptions.all().data
		for subscription in current_subscriptions:
			subscription_plan_type = get_plan_type(subscription.plan.id)
			if subscription_plan_type == plan_type:
				return subscription

	def cancel(self, plan_type):
		subscription = self.get_subscription(plan_type)
		if subscription is not None:
			response = subscription.delete(at_period_end=True)
			cancelled.send(sender=self, stripe_response=response)

		try:
			setattr(self.current_subscription, '%s_status' % plan_type, False)
	...
	def save_card(self, cu=None):
		...
		self.card_stripe_id = active_card.id
		self.save()
	...
	def update_subscription(self, subscription, plan_id, prorate=False):
		"""Update subscription on stripe and return response.
		Update subscription only when the plan has changed."""
		if subscription.plan.id != plan_id:
			subscription.plan = plan_id
			subscription.prorate = prorate
			return subscription.save()

	def get_plan_id(self, plan_type, payment):
		return '%s_%d' % (plan_type, payment)

	def subscribe(self, plan_type, payment):
		def set_subscription_status():
			data = {plan_type + '_status': True}
			try:
				current_subscription_id = self.current_subscription.pk
				CurrentSubscription.objects.filter(pk=current_subscription_id).update(**data)
			except CurrentSubscription.DoesNotExist:
				data.update({
					'customer': self
				})
				CurrentSubscription.objects.create(**data)

		cu = self.stripe_customer
		plan_id = self.get_plan_id(plan_type, payment)
		subscription = self.get_subscription(plan_type)
		if subscription is not None:
			resp = self.update_subscription(subscription, plan_id, prorate=False)
		else:
			resp = cu.subscriptions.create(plan=plan_id)

		# charge immediately
		self.send_invoice()

		subscription_made.send(sender=self, plan=plan_id, stripe_response=resp)

		# sync invoices for payment history
		self.sync_invoices()
...
class Payment(models.Model):
	stripe_id = models.CharField(max_length=255, null=True, blank=True)  # None when paid is True
	created_at = models.DateTimeField(default=timezone.now)
	appointment = models.ForeignKey('healers.Appointment', related_name='payments')
	date = models.DateField()
	amount = models.PositiveIntegerField()  # 0 when paid is True
	paid = models.BooleanField(default=False)  # ability to mark paid even if it's not paid

	def __unicode__(self):
		return '%s - %2.2f' % (self.appointment, self.amount / 100.0)
